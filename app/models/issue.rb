...
class Issue < ActiveRecord::Base
    ...
    (usr || User.current).allowed_to?(:view_issues, self.project) do |role, user|
      visible =
        if user.logged?
          ...
          when 'own_watch_contributed'
            self.author == user || user.is_or_belongs_to?(assigned_to) || self.watched_by?(user) || self.journals.where(user_id: user, rolled_back: false).any?
          else
            false
          end
        else
  ...
  def workflow_rule_by_attribute(user=nil)
    ...
    user_real = user || User.current
    roles = user_real.admin ? (project.present? ? project.members.map(&:roles).flatten.uniq : Role.all.to_a) : user_real.roles_for_project(project)
    roles = roles.select(&:consider_workflow?)
    return {} if roles.empty?

    result = {}
    workspace_id = project.nil? ? nil : project.workspace_id
    workflow_permissions = WorkflowPermission.where(:tracker_id => tracker_id, :old_status_id => status_id, :role_id => roles.map(&:id), :workspace_id => workspace_id).to_a
